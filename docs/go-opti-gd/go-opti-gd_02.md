...
> 原文：[https://goperf.dev/](https://goperf.dev/)
...
Go 故意隐藏了其大部分底层控制。你不会得到显式的内存管理，也无法微观管理线程。你得到的是足够的可见性来推理分配、调度和I/O行为。结合良好的工具，这通常足以构建快速、可预测的系统。本系列文章就停留在这一领域。实用杠杆，而非理论完美。

这里的目标不是巧妙，而是编写在流量激增时保持快速且六个月后不会崩溃的枯燥代码。

## 第1部分。[Go性能常见模式](01-common-patterns/)
