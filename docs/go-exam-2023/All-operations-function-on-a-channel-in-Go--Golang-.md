...
> 来源：[`golangbyexample.com/all-operations-channel-golang/`](https://golangbyexample.com/all-operations-channel-golang/)
...
在上面的程序中，我们创建了一个通道。然后我们在一个 goroutine 中调用了**sum**函数。在主函数中，我们向通道发送了 3 个值，之后关闭了通道，表示不能再向通道发送更多的值。**sum**函数使用 for 循环迭代通道并计算总和。
...
在上面的程序中创建了一个容量为 1 的通道。然后我们向通道发送了一个值。在第一次接收中，**ok**变量为 true，因为通道未关闭。在第二次接收中，ok 变量为 false，因为通道已关闭。
